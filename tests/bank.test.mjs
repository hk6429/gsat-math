...
test("108～115 學年度選擇題附官方各選項畫記率，未公開年份不反推", () => {
  for (const exam of bank) {
    for (const q of exam.questions) {
      const hasPublishedRates = (exam.year >= 108 && exam.year <= 110 && q.no <= 13) || (exam.year >= 111 && q.no <= 12);
...
test("逐字 LaTeX 題目資料具有題幹、選項、解析與官方頁碼", () => {
  ...
  for (const year of [108, 109, 110]) {
    for (let no = 1; no <= 20; no += 1) {
      assert.ok(mathContent[`${year}M-${no}`], `缺少 ${year}M 第 ${no} 題結構化資料`);
    }
  }
  assert.equal(Object.keys(mathContent).length, 260);
});
