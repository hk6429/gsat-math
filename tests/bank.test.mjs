...
test("官方答案逐題固定", () => {
  const expected = {
    ...
    "87M": ["4","1","4","2","1,3,5","1,2,4,5","3,4","2,3,5","1,2,3","1,2,4","150","9","20","24","24","(1-√5)/2","50√7","2x+5","(x-2)^2/9+y^2/5=1","109%"],
    ...
    "83M": ["4","2","4","4","5","2","4","3,5","1,5","3,4,5","(-2,1/16)","20","(-8/3,16/3)","(7,-2)","-1±2√6/3","160π/11","(1,5/2,5/2)","8","4/5","-7"]
  };
  for (const exam of bank) {
...
test("首頁與查題頁都載入解析及官方選項統計介面", () => {
  ...
  assert.match(homepage, /id="paperPanel"/);
  assert.match(homepage, /id="paperQuestionList"/);
  assert.match(homepage, /id="paperLinkBtn"/);
  assert.match(homepage, /id="paperWordBtn"/);
  ...
  assert.match(app, /function createPaperLink/);
  assert.match(app, /function downloadPaperWord/);
  assert.match(app, /function loadLinkedQuiz/);
  assert.match(renderer, /各選項作答分析/);
  assert.match(renderer, /逐選項解題分析/);
...
test("逐字 LaTeX 題目資料具有題幹、選項、解析與官方頁碼", () => {
  ...
  for (const year of [83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110]) {
    for (let no = 1; no <= 20; no += 1) {
      assert.ok(mathContent[`${year}M-${no}`], `缺少 ${year}M 第 ${no} 題結構化資料`);
    }
  }
  assert.equal(Object.keys(mathContent).length, 760);
});
